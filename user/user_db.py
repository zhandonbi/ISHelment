...
class User(object):
    ...
    def get_field_content(self, fields, key:str, value):
        ...
        if fields is None:
            ...
            FField = ','.join(fields)
        sql = 'SELECT {} FROM {} WHERE {} = {}'.format(FField, self.tableUser, key, value)
    ...
    def check_userName(self,userName):
        res = self.get_field_content(['"workID"'],'name','"{}"'.format(userName))
        if len(res) is 0:
            return None
        else:
            return res[0][0]
    ...
    def del_user(self, userID):
        if self.check_userID(userID) is None:
            ...
            sql = 'DELETE FROM {} WHERE workID = {}'.format(self.tableUser, userID)
            try:
                self.cur.execute(sql)
                self.operator.commit()
            except Exception as e:
    ...
    def find_user_by_id(self,userID):
        sql = 'SELECT * FROM {} WHERE `workID` = {}'.format(self.tableUser, userID)
        try:
            self.cur.execute(sql)
            res = self.cur.fetchall()
            if len(res)  != 0:
                return self.returnValue(True, list(res[0]))
            else:
    ...
    def find_user_by_name(self,userName):
        sql = 'SELECT * FROM {} WHERE `name` = \'{}\''.format(self.tableUser, userName)
        try:
            self.cur.execute(sql)
            res = self.cur.fetchall()
            if len(res) != 0:
                final_res = []
                for now_line in res:
                    final_res.append(list(now_line))
